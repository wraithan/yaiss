            ...
            if message['version'] == 1:
                if message['type'] == 'privmsg':
                    Line.objects.create(time=datetime.now(),
                                        ...
                                        nick=Nick.objects.get_or_create(name=message['data']['sender'], server=Server.objects.get_or_create(name='irc.freenode.net'))
                                        channel=Channel.objects.get_or_create(name=message['data']['channel'], server=Server.objects.get_or_create(name='irc.freenode.net'))
